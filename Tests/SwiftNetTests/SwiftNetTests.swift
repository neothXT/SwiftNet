...
final class SwiftNetTests: XCTestCase {
	...
	func testBadResponseFetchNoInternetConnection() throws {
		...
		provider.testRaw(.posts, usingMocks: false, storeIn: &subscriptions) {
			if let error = $0 as? SNError, error.type == .noInternetConnection {
				expectation.fulfill()
			}
		}
	...
	func testNetworkMonitor() throws {
		...
		SNNetworkMonitor.shared.publisher()
			.sink { status in
	...
	func testUrlEncodedBody() throws {
		...
		SNDataEncoder.encode(endpoint.data, boundary: nil, request: &urlRequest)
	...
	func testMockedFetch() throws {
		...
		SNProvider<RemoteEndpoint>().test(.posts, responseType: [Post].self, usingMocks: true, storeIn: &subscriptions) { data in
			if (data ?? []).count == 5 {
				expectation.fulfill()
			}
		}
	...
	func testMockedPost() throws {
		...
		SNProvider<RemoteEndpoint>().test(.post(post), responseType: [Post].self, usingMocks: true, storeIn: &subscriptions) { data in
			if (data ?? []).count == 6 {
				expectation.fulfill()
			}
		}
